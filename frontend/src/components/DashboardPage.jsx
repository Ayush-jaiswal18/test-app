  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-8">
        <h1 className="text-4xl font-bold text-gray-800">My Tests</h1>
        <Link to="/create-test" className="bg-blue-600 text-white px-6 py-2 rounded-lg hover:bg-blue-700 transition">
          Create New Test
        </Link>
      </div>
      {tests.length === 0 ? (
        <p className="text-center text-gray-500 mt-12">You haven't created any tests yet.</p>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {tests.map((test) => {
            const shareableLink = `${window.location.origin}/test/${test._id}`;
            return (
              <div key={test._id} className="bg-white p-6 rounded-lg shadow-lg hover:shadow-xl transition-shadow">
